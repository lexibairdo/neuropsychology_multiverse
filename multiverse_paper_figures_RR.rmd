---
title: "Multiverse: Renamed Figures and Tables"
...
mv_complete <- mv_complete %>%
  rename(          
    `Age` = Main.AgeYrs,
    `Age^2` = Main.AgeYrs_Squared,
    `Race` = Demographics.RaceCatCOBRIT,
    `Income` = Demographics.AnnualIncomePreInjOrdinal,
    `Education` = Demographics.EduLvlUSATypCOBRITOrdinal,
    `Gender` = Demographics.GenderTyp
  )

#rename for final revision
#mv_complete <- mv_complete %>%
  #rename(
    #`BSI Global` = `Brief Symptom Inventory (BSI) Global Severity Score`
  #_

#Rename BSI Depression Score to BSI Depression, and BSI Anxiety Score to BSI Anxiety
#mv_complete <- mv_complete %>%
  #rename(
    # `BSI Depression` = `BSI Depression Score`,
    #`BSI Anxiety` = `BSI Anxiety Score`
  #)
...
PSI_specs <- specr::setup(data = mv_complete,            
               ...
               model = c("lm"), 
               controls = c("`Age`", "`Age^2`", "`Race`", "`Income`", "`Education`", "`Gender`"))
...
plot(PSI_results) +
  ...
  ggtitle("Specification Curve of the effects of TBI Severity on Processing Speed")

ggsave("PSI_outcome_curve_RR_demo.jpg", width = 30, height = 12, dpi = 600)
...
# Generate the specification curve plot with renamed variables
PSI_outcome_curve <- plot(PSI_results) +
  labs(
    x = "Predictor Variables",
    y = "Outcome Variables",
    title = "Specification Curve for Processing Speed Outcomes")

PSI_outcome_box <- plot(PSI_results, type = "boxplot")
...
#Save the plot 
ggsave("QOL_outcome_curve_RR.jpg", width = 8, height = 14, dpi = 600)

qol_outcome_box <- plot(qol_specs_results, type = "boxplot")
...
mood_outcome_specs <- specr::setup(data = mv_complete,            
               y = c("`BSI Global`", "`BSI Depression`", "`BSI Anxiety`"),              
               x = c("`Glasgow Coma Scale (GCS)`", "`Post Traumatic Amnesia (PTA)`", "`Galveston Orientation and Amnesia Test (GOAT)`"),           
               model = c("lm"), 
               controls = c("`Age`", "`Age^2`", "`Race`", "`Income`", "`Education`", "`Gender`"))
...
ggsave("Mood_outcome_curve_RR.jpg", width = 8, height = 14, dpi = 600)


mood_outcome_box <- plot(mood_outcome_results, type = "boxplot")
...
psi_descriptive_gt <- mv_complete |>
  tbl_summary(by = `Gender`, include = c(`Processing Speed Index Standardized Score`,`Symbol Search Standardized Score`, `Digit Symbols Standardized Score`)) |>
  add_p() |>
  as_gt() |>
  tab_header(
...
qol_descriptive_gt <- mv_complete |>
  tbl_summary(by = `Gender`, include = c(`Satisfaction with Life Scale (SWLS) Total Score`, `Disability Rating Scale (DRS) Total Score`, `SWLS-DRS Composite Score`)) |>
  add_p() |>
    as_gt() |>
    tab_header(
...
mood_descriptive_gt <- mv_complete |>
  tbl_summary(by = `Gender`, include = c(`Brief Symptom Inventory (BSI) Global Severity Score`, `BSI Depression Score`, `BSI Anxiety Score`)) |>
    add_p() |>
        as_gt() |>
        tab_header(
...
tbi_severity_gt <- mv_complete |>
  tbl_summary(by = `Gender`, include = c(`Glasgow Coma Scale (GCS)`, `Post Traumatic Amnesia (PTA)`, `Galveston Orientation and Amnesia Test (GOAT)`)) |>
    add_p() |>
        as_gt() |>
        tab_header(
...
# Create display versions of variables for the table
